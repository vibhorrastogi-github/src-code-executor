 ...
 * 
 */
public abstract class CodeExecuteServiceDepricated implements InitializingBean {

	private static Logger LOGGER = LoggerFactory
			.getLogger(CodeExecuteServiceDepricated.class);
	...
	protected void execute(final CodeExecuteRequest codeExecuteRequest,
		...
		try {
			...
			process = executeCmd(command, codeExecuteRequest);
	 ...
	 * 
	 */
	private void destroyProcess(final Process process) {
		try {
			LOGGER.info("destroying process...");
			process.destroy();
			LOGGER.info("process desctroyed...");
		} catch (final Exception e) {
			LOGGER.error("unable to destroy process", e);
		}
	}
...
//		pb.command(new String[] { "cmd", "/c", command });
		pb.command(new String[] { "cmd", "/C start /B /belownormal", command });
